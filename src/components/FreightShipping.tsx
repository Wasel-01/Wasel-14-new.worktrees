...
export function FreightShipping() {
  ...
  const handleQuote = async () => {
    try {
      const originInput = (document.querySelector('input[placeholder="City or Port"]') as HTMLInputElement)?.value;
      const destinationInput = (document.querySelectorAll('input[placeholder="City or Port"]')[1] as HTMLInputElement)?.value;
      const pickupDateInput = (document.querySelector('input[type="date"]') as HTMLInputElement)?.value;
      ...
      // Create freight shipping request
      const trip = await dataService.createTrip({
        from: originInput,
        to: destinationInput,
        departure_date: pickupDateInput || new Date().toISOString().split('T')[0],
        departure_time: '09:00',
        trip_type: 'package',
        price_per_seat: 200, // Freight pricing
        total_seats: 1,
        available_seats: 1,
      });

      toast.success('Quote request sent! Carriers will respond shortly. Estimated cost: AED 200-500.');
    } catch (error: any) {
             ...
             <CardContent className="p-6">
               ...
               <ul className="space-y-4 text-sm text-slate-300">
                 ...
                 <li className="flex gap-3">
                   ...
                   <span>Cargo Insurance up to AED 100k</span>
                 </li>
                 <li className="flex gap-3">
            ...
            <CardContent>
              <div className="space-y-4">
                ...
                <div className="flex justify-between items-center text-sm">
                  ...
                  <span className="font-mono font-bold">AED 2,400</span>
                </div>
              </div>
            </CardContent>
          </Card>
  ...
  return (
    <ServiceTabs
      ...
      mainServiceTitle="Freight Shipping"
      mainServiceIcon={<Truck className="w-4 h-4" />}
      showAllTabs={true}
    />
  );
}
