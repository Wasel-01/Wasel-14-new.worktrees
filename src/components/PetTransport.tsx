...
export function PetTransport() {
  ...
  const handleSearch = async () => {
    try {
      // In a real app, this would search for pet-friendly trips
      // For now, create a pet transport booking
      const fromInput = (document.querySelector('input[placeholder="Pickup Address"]') as HTMLInputElement)?.value;
      const toInput = (document.querySelector('input[placeholder="Dropoff Address"]') as HTMLInputElement)?.value;
      const dateTimeInput = (document.querySelector('input[type="datetime-local"]') as HTMLInputElement)?.value;
      
      if (!fromInput || !toInput || !dateTimeInput) {
        toast.error('Please fill in all journey details');
        return;
      }
      ...
      const trip = await dataService.createTrip({
        ...
        departure_time: time,
        trip_type: 'wasel',
        price_per_seat: 40, // Pet transport pricing
        total_seats: 1,
        available_seats: 1,
        preferences: {
      ...
      toast.success('Pet transport booking confirmed! Pet-friendly captain will contact you soon.');
    } catch (error: any) {
      toast.error(error.message || 'Failed to book pet transport');
    }
  };
  ...
  return (
    <ServiceTabs
      ...
      mainServiceTitle="Pet Transport"
      mainServiceIcon={<PawPrint className="w-4 h-4" />}
      showAllTabs={true}
    />
  );
}
