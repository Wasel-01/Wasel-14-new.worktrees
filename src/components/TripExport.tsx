...
export function TripExport() {
  ...
  const handleExport = async () => {
    ...
    try {
      ...
      // Fetch trips with driver profile joined
  ...
  const exportToCSV = (trips: any[]) => {
    ...
    const rows = trips.map(trip => [
      ...
      trip.status || 'N/A',
      (trip.driver as any)?.full_name || 'N/A', // Driver name from joined profile
    ...
    link.setAttribute('download', `wassel-trips-${period}-${Date.now()}.csv`);
  ...
  const exportToPDF = (trips: any[]) => {
    ...
    const htmlContent = `
      ...
      <html>
      <head>
        <title>Wassel Trip History</title>
        <style>
      ...
      <body>
        ...
        <div class="summary">
          ...
          <p>Total Spent: AED ${trips.reduce((sum, t) => sum + (t.fare || 0), 0).toFixed(2)}</p>
          ...
          <tbody>
            ${trips.map(trip => `
              <tr>
                ...
                <td>AED ${trip.fare?.toFixed(2) || '0.00'}</td>
