...
export function useAITracking() {
  ...
  const trackEvent = useCallback(async (
    ...
    try {
      ...
      try {
        const { analyticsService } = await import('../services/integrations');
        analyticsService.track('ai_interaction', {
          eventType: trackingData.eventType,
          feature: trackingData.eventData?.feature,
          confidence: trackingData.aiResponse?.confidence,
          latency: trackingData.aiResponse?.latency,
        });

        // Also send to backend for detailed analytics
